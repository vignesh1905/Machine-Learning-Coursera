...
%     
      ...
      y_matrix = y_eye(y,:); % This is done to convert y into a matrix containing ones at the places num_labels (refer pdf and tutorial)
      a1 = X;
      z2 = a1 * Theta1'; 
      a2 = sigmoid(z2);
      ...
      J = ((1/m)* sum(sum((-y_matrix .* log(a3))-((1-y_matrix) .* log(1-a3))))); % Here since y and h = a3 are both matrix and not vectors we have to do element wise calculation and then find double sum which gives us a single value for the cost function
	  ...
	  J =  J + j1 + j2 ; 
	  % Back Propogation
	  d3 = a3 - y_matrix ; %Computing small delta (errors) 
